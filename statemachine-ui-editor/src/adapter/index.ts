import {getBpmnId} from '@logicflow/extension/src/bpmn-adapter/bpmnIds'
import {handleAttributes, lfJson2Xml} from './json2xml'
import {lfXml2Json} from '@logicflow/extension/src/bpmn-adapter/xml2json'
...
export interface EdgeEventConfig {
...
function convertLf2ProcessData(
    ...
    const sequenceFlow = data.edges.map((edge: EdgeConfig) => {
        ...
        //生成事件
        if (edge.events) {
            let tmp = edge.events.map((event: any) => {
                return {
                    '-event': event.event,
                    '-type': event.type,
                    '-action': event.action,
            ...
            edgeConfig['bpmn:extensionElements']['bpmn:taskListener'] = tmp;
        }
        if (edge.expression) {
            edgeConfig['conditionExpression'] = {
                '-xsi:type': "tFormalExpression",
                "#cdata-section": edge.expression
            }
        }
        if (edge.properties) {
 ...
 */
function convertBpmn2LfData(bpmnData: any) {
        ...
        Object.keys(process).forEach((key) => {
            if (key.indexOf('bpmn:') === 0) {
                ...
                if (key == "bpmn:extensionElements") {
                    let extNode = getExtensionConfig(value)
...
//获取扩展节点
function getExtensionConfig(value: any) {
    let properties;
    const value2 = value["camunda:properties"]
    if (value2) {
        let propertys = value2["camunda:property"]
        if (propertys) {
            if (!Array.isArray(propertys)) {
                propertys = [propertys];
            }
            propertys.forEach((property: any) => {
                if (!properties) properties = {}
                properties[property["-name"]] = property["-value"]
            })
        }
    }
    const nodeConfig: ExtendConfig = {
        id: "",
        properties,
    }
    return nodeConfig
...
function getEdgeConfig(edgeValue, processValue): EdgeConfig {
    ...
    //增加事件
    let events;
    if (processValue['bpmn:extensionElements']) {
        let tmpevnts = processValue['bpmn:extensionElements']["bpmn:taskListener"];
        if (tmpevnts) {
            if (!Array.isArray(tmpevnts)) {
                tmpevnts = [tmpevnts];
            }
            events = tmpevnts.map((event) => {
                return {
                    event: event['-event'],
                    type: event['-type'],
                    action: event['-action'],
...
class BpmnXmlAdapter extends BpmnAdapter {
    ...
    //顶层的process数据
    processData = {properties: {}, id: ""}

    //连接线
    edges: EdgeConfig[] = [];

    //设置值
    setProcessProperties = (data: any) => {
        this.processData.properties = data;
    }

    //更新表达式
    setExpression = (id: string, expression: string) => {
        let data = this.loginFlow.getGraphRawData()
        this.initEdges(data.edges);
        let edge = this.edges.find((item) => {
            return item.id == id;
        })
        if (!edge) {
            return;
    ...
    //更新事件
    setEdgeEvent = (id: string, events: EdgeConfig[]) => {
        let data = this.loginFlow.getGraphRawData()
        this.initEdges(data.edges);
        let edge = this.edges.find((item) => {
            return item.id == id;
    ...
    initEdges = (gedges: EdgeConfig[]) => {

        gedges.forEach((item) => {
            let findItem = this.edges.find((edge) => {
    ...
    backFillEdges = (data: BpmnConfig) => {
        ...
        data.edges.forEach((item) => {
            ...
            if (findItem) {
                //赋值，并取消Proxy对象
                if (findItem.events) {
                    item.events = JSON.parse(JSON.stringify(findItem.events));
                }
                item.expression = findItem.expression;
            }
        })
    }
    ...
    adapterXmlIn = (bpmnData: any) => {
        ...
        const data = this.adapterIn(json) as BpmnConfig;
        this.processData = {properties: data.properties, id: data.id};
        this.edges = data.edges;
        return data;
    }
    adapterXmlOut = (data: BpmnConfig, retainedFields?: string[]) => {
        data["properties"] = this.processData.properties;
        //给连接线增加属性
        this.backFillEdges(data);
